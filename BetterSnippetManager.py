    ...
    def __list_all_snippets(self, path, all_snippets, extension):
        ...
        for item in os.listdir(path):
            if os.path.isdir(os.path.join(path, item)):
                self.__list_all_snippets(
                    os.path.join(path, item), all_snippets, extension
                )
    ...
    def run(self):
        ...
        if len(self.all_snippets) == 0:
            return self.error_message(
                "BetterSnippetManager: Didn't find any '{}' in the User "
                "package".format(extension)
            )
        self.window.show_quick_panel(
            self.all_snippets, self.on_done, 0, 0, self.on_highlighted
        )
...
class BetterSnippetManagerCreateCommand(sublime_plugin.TextCommand):
    ...
    def set_trigger(self, trigger):
        ...
        self.window.show_input_panel(
            "Description: ", "", self.set_description, None, None
        )
    ...
    def set_description(self, description):
        ...
        view = self.window.show_input_panel(
            "Scope: ", scopes, self.set_scopes, None, None
        )
        sel = view.sel()
        sel.clear()
    ...
    def ask_file_name(self):
        ...
        sel.clear()
        sel.add(
            sublime.Region(0, len(os.path.splitext(os.path.basename(self.trigger))[0]))
        )
    ...
    def make_snippet(self, file_name):
        ...
        snippet_content = template.format(
